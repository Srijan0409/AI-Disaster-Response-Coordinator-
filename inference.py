...
MANDATORY env vars:
    ...
    ENV_URL          The deployed HF Space URL for the environment server.
...
ENV_URL      = os.getenv("ENV_URL", "http://localhost:8000")   # override with your HF Space URL
...
def choose_action(client: OpenAI, obs_dict: dict, step: int, task_name: str) -> int:
    """
...
def run_task(client: OpenAI, task_name: str, difficulty: str) -> dict:
    """
    Run one full episode for the given task.
    Connects to the running environment server via ENV_URL.
    Returns {"task": str, "score": float, "steps": int, "success": bool}
    """
    rewards: List[float] = []
    steps_taken = 0
    score = 0.0
    ...
    # ── FIX: Connect to deployed HF Space via base_url (synchronous) ──────────
    # DisasterEnv is a synchronous EnvClient — use plain `with`, not `async with`
    # Set ENV_URL to your HF Space URL e.g. https://YOUR-USERNAME-disaster-env.hf.space
    with DisasterEnv(base_url=ENV_URL) as env:
        try:
            # Reset with the right difficulty (seed=42 for reproducibility)
            result = env.reset(difficulty=difficulty, seed=42)
            ...
            for step in range(1, MAX_STEPS + 1):
                ...
                try:
                    result = env.step(DisasterAction(allocate_to=victim_id))
                    obs    = result.observation
...
def main() -> None:
    ...
    for task_name, difficulty in TASKS:
        result = run_task(client, task_name, difficulty)
